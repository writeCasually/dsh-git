...
const st = useStore()
return React.createElement('button', { className: 'gb', onClick: () => setState({ open: !st.open, sessionId: st.sessionId || props.sessionId }), title: 'Git 状态与操作' }, st.open ? 'Git ✓' : 'Git')
}
function Panel() {
const st = useStore()
...
const pick = (p, g) => { setSel({ staged: g === 's', path: p }); setTab('变更') }
...
React.createElement('button', { className: 'gb' + (tab === '变更' ? ' go' : ''), onClick: () => setTab('变更') }, '变更'),
React.createElement('button', { className: 'gb' + (tab === '提交' ? ' go' : ''), onClick: () => setTab('提交') }, '提交')),
tab === '变更' ? React.createElement(ChangesTab, { sel: sel, onSel: setSel, onPick: pick }) : null,
...
host.call('p0.diff', { sessionId: state.sessionId, staged: props.sel.staged, path: props.sel.path || undefined }).then((r) => {
...
try { setOut(await host.call('p0.commit', { sessionId: state.sessionId, message: msg })) } catch (e) { setOut({ error: String((e && e.message) || e) }) }
setArm(false)
}, [msg])
return React.createElement('div', null,
React.createElement('div', { className: 'gs' }, '提交'),
React.createElement('textarea', { className: 'gi', rows: 3, placeholder: 'commit message', value: msg, onChange: (e) => setMsg(e.target.value) }),
React.createElement('div', { className: 'gr' },
!arm ? React.createElement('button', { className: 'gb', disabled: !msg.trim(), onClick: () => setArm(true) }, '提交全部变更（add -A）') : null,
arm ? React.createElement('button', { className: 'gb go', onClick: doIt }, '确认提交') : null,
arm ? React.createElement('button', { className: 'gb', onClick: () => setArm(false) }, '取消') : null),
out ? (out.error ? React.createElement('div', { className: 'ge' }, out.error) : React.createElement('div', { className: 'go2' }, '已提交：' + out.output)) : null)
}
