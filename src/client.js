...
/** Recent turns with file changes (panel-side summary; independent of turnTail rendering). */
...
/** Codex-style per-turn modification card: file list with +N -M, expandable to the diff. */
function TurnTailCard(props) {
  const turn = props.matched && props.matched.turn
  const sessionId = (props.matched && props.matched.sessionId) || state.sessionId
  ...
  React.useEffect(() => {
    ...
    setOpen({})
    setErr(null)
    if (sessionId == null || turn == null) { setFiles([]); return }
    callRemote('dshGit/turnDiff', { sessionId, turn, upToSeq: (props.matched && props.matched.seq) })
      .then((r) => { if (alive) setFiles(r.files || []) })
      .catch((e) => { if (alive) { setFiles([]); setErr(String((e && e.message) || e)) } })
    return () => { alive = false }
  }, [sessionId, turn])
  if (!files || files.length === 0) return null
  const totalAdd = files.reduce((sum, f) => sum + f.add, 0)
  const totalDel = files.reduce((sum, f) => sum + f.del, 0)
  return React.createElement(
    'div',
    { className: 'ta dshgit-card', 'data-dsh-git': 'turn-tail' },
      React.createElement('div', { className: 'tm' },
        React.createElement('span', { className: 'dshgit-tag' }, 'dsh-git'),
        ` 本回合修改 ${files.length} 个文件 · +${totalAdd} −${totalDel}`),
      files.map((f) => {
        ...
        return React.createElement(
          ...
          opened && f.hunks.length
            ? React.createElement(
                'div',
                { className: 'tl' },
                f.hunks.map((h, i) => React.createElement('pre', { className: 'px', key: i }, hunkText(h))),
              )
            : null,
        )
      }),
      ...
      try {
        const dispose = slots.register({
          ...
          select: (owner) => {
            ...
            return (typeof n === 'number') ? { turn: n, seq: owner.seq } : null
          },
        }, TurnTailCard)
        return dispose
      } catch (error) {
